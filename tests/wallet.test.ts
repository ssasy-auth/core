...
describe("[Wallet Class Test Suite]", () => {
  ...
  describe("generateChallengeResponse() with required signature", () => {
    ...
    it("should throw an error if challenge signature is invalid", async () => {
      const encryptedChallengesWithInvalidSignatures = [
        {
          ...encryptedChallenge, signature: "invalid"
        },
        {
          ...encryptedChallenge, signature: undefined as any
        },
        {
          ...encryptedChallenge, signature: null as any
        }
      ];

      for (const encryptedChallenge of encryptedChallengesWithInvalidSignatures) {
        try {
          const encryptedChallengeString = await SerializerModule.serializeCiphertext(encryptedChallenge);
    ...
    it("should return solution if valid signature is provided", async () => {
      ...
      const response = await friendWallet.verifyChallengeResponse(encryptedChallengeResponseString)
